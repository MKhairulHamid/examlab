...
const aifC01Course = {
  ...
  sessions: [
    // ───────────────────────── DOMAIN 1 ─────────────────────────
    {
      ...
      duration: 30,
      summary: 'Build the vocabulary. Distinguish AI, ML, deep learning, generative AI, and agentic AI, and understand how models are trained and used.',
      objectives: [
      ...
      sections: [
        {
          heading: 'The nested hierarchy',
          body: 'These terms are concentric circles, not synonyms. Each sits inside the previous one.',
          bullets: [
            'Artificial Intelligence (AI) — the broad field of machines performing tasks that normally require human intelligence.',
            'Machine Learning (ML) — a subset of AI where systems learn patterns from data instead of being explicitly programmed.',
            'Deep Learning (DL) — a subset of ML that uses multi-layer neural networks; powers vision, speech, and language tasks.',
            'Generative AI (GenAI) — deep-learning models that create new content (text, images, audio, code).',
            'Agentic AI — systems that use a model to plan, call tools, and act autonomously toward a goal.',
          ],
        },
        {
          heading: 'Key building blocks',
          bullets: [
            'Neural network — layers of interconnected "neurons" that learn weighted relationships in data.',
            'Model — the trained artifact that maps inputs to outputs. Algorithm — the procedure used to train it.',
            'Computer vision (CV) — interpreting images/video. NLP — understanding and generating human language.',
            'Bias — systematic error producing unfair results. Fairness — equitable outcomes across groups.',
            'Fit — overfitting (memorizes training data, fails on new data) vs underfitting (too simple to capture patterns).',
          ],
        },
        {
          heading: 'Training vs inferencing',
          body: 'Training is the (expensive, one-time-ish) process of learning weights from data. Inferencing is using the trained model to make predictions on new data.',
          table: {
            headers: ['Inference type', 'When to use'],
            rows: [
              ['Real-time', 'Low-latency, synchronous predictions (chatbot, fraud check at checkout)'],
              ['Batch', 'Large volumes processed on a schedule; latency not critical (nightly scoring)'],
              ['Asynchronous', 'Large payloads or long-running jobs; results returned later'],
              ['Serverless', 'Intermittent/spiky traffic; no infrastructure to manage, scales to zero'],
            ],
          },
        },
        {
          heading: 'Learning methods & data types',
          bullets: [
            'Supervised learning — trained on labeled data (input → known output). Used for classification & regression.',
            'Unsupervised learning — finds structure in unlabeled data. Used for clustering & anomaly detection.',
            'Reinforcement learning — an agent learns by trial-and-error using rewards.',
            'Data types: structured/tabular, time-series, image, text, audio; labeled vs unlabeled.',
          ],
        },
      ],
      keyTerms: [
        ...
        { term: 'Labeled data', def: 'Data where each example includes the correct answer; required for supervised learning.' },
        { term: 'Agentic AI', def: 'AI that can plan, use tools, and take multi-step actions toward a goal autonomously.' },
      ],
      awsServices: [],
      examTips: [
        'If a question contrasts "learns from data" vs "explicitly programmed," it is testing ML vs traditional software.',
        'Serverless and asynchronous inference are favored when traffic is spiky or payloads are large.',
        'Supervised = labeled; unsupervised = unlabeled. This single fact answers many D1 questions.',
      ],
      sample: {
        ...
        correct: 1,
        explanation: {
          summary: 'There are no predefined labels (segments), so the model must discover natural groupings on its own — that is clustering, an unsupervised technique.',
          perOption: [
            'Classification needs labeled examples of each segment, which the retailer does not have.',
            'Correct — clustering is unsupervised and finds natural groupings without labels.',
            'Reinforcement learning fits sequential decision-making with rewards, not segmentation.',
            'Regression predicts a continuous numeric value (e.g., spend), not groupings.',
          ],
