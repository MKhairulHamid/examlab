...
export default function CommunityVideoPanel({ videos, currentUserId }) {
  ...
  return (
    <div>
      <p style={{ fontSize: '0.8125rem', color: '#475569', lineHeight: 1.55, margin: '0 0 1rem' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '0.875rem 1.125rem' }}>
          ...
          {v.note && (
            <p style={{ fontSize: '0.8125rem', color: '#475569', lineHeight: 1.55, margin: '0.6rem 0 0' }}>{v.note}</p>
          )}
        </div>

        {/* Footer: report */}
        <div style={{ padding: '0.6rem 1.125rem', borderTop: '1px solid #f1f5f9', background: '#fafbfd', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem' }}>
          <span style={{ fontSize: '0.6875rem', color: '#94a3b8', lineHeight: 1.5 }}>
            Submitted by a learner and reviewed by our team. CloudExamLab does not host this video.
          </span>
          <button
            onClick={() => setReportFor(v)}
            style={{ flexShrink: 0, fontSize: '0.6875rem', fontWeight: 700, color: '#94a3b8', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', whiteSpace: 'nowrap' }}
          >
            Report
          </button>
        </div>
      ...
      >
        {status === 'done' ? (
          ...
          <>
            ...
            <p style={{ fontSize: '0.8125rem', color: '#64748b', margin: '0 0 1rem', lineHeight: 1.55 }}>
              Tell us what's wrong with "{video.title}". Reports are private.
            </p>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', marginBottom: '1rem' }}>
              {REPORT_REASONS.map(r => (
                <label key={r.value} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.875rem', color: '#1e293b', cursor: 'pointer' }}>
            ...
            <textarea
              ...
              rows={3}
              style={{ width: '100%', boxSizing: 'border-box', borderRadius: '0.6rem', border: '1.5px solid #e2e8f0', padding: '0.6rem 0.75rem', fontSize: '0.875rem', fontFamily: 'inherit', resize: 'vertical', marginBottom: '0.75rem' }}
            />
